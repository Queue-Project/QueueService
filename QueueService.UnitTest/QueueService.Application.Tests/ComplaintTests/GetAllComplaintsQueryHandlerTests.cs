...
public class GetAllComplaintsQueryHandlerTests
{
     private readonly Mock<ILogger<GetAllComplaintsQueryHandler>> _mockLogger;
    private readonly QueueDbContext _dbContext;
    private readonly Mock<IHttpContextAccessor> _mockAccessor;
    private readonly Mock<IUserService> _mockUserService;
    private readonly GetAllComplaintsQueryHandler _handler;

    public GetAllComplaintsQueryHandlerTests()
    {
        _mockLogger = new Mock<ILogger<GetAllComplaintsQueryHandler>>();
        _dbContext = TestDbContextFactory.Create();
        _mockAccessor = new Mock<IHttpContextAccessor>();
        _mockUserService = new Mock<IUserService>();
        _handler = new GetAllComplaintsQueryHandler(_mockLogger.Object, _dbContext, _mockAccessor.Object,
    ...
    public async Task Handler_Should_Return_AllComplaints_When_User_Is_Customer_Successfully()
    {
        ...
        var query = new GetAllComplaintsQuery(1);
        ...
        var firstComplaint = result.Items.FirstOrDefault();
        firstComplaint!.Id.ShouldBe(1);
    }
    
    [Fact]
    public async Task Handler_Should_Return_AllComplaints_When_User_Is_Employee_Successfully()
    {
        //Arrange
        var complaints = TestDataSeeder.CreateComplaints();
        await _dbContext.Complaints.AddRangeAsync(complaints, CancellationToken.None);
    ...
    [Fact]
    public async Task Should_Return_ComplaintEmptyList()
    {
        //Arrange
        var expectedResponse = new Claim("id", "1");
